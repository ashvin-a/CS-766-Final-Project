...
function CameraRig({ curve, progressRef }: RigProps) {
  const { camera } = useThree()

  // All scratch objects pre-allocated — zero heap allocation inside useFrame
  ...
  useFrame(() => {
    ...
    // Use getPoint(t, target) to write into pre-allocated vectors — no allocation
    ...
    _desiredPos.current
      ...
      .addScaledVector(_forward.current, -9.5)
      .addScaledVector(_camUp.current, 3.8)
...
export function TourScene({ curveData, progressRef, activeIdx }: Props) {
  ...
  return (
    <Canvas
      ...
      // Cap pixel ratio at 2× — 3× retina triples fill cost for minimal visual gain
      dpr={[1, 2]}
      onCreated={({ gl, scene }) => {
